...
read_csvfile=pd.read_csv('/home/fly/PycharmProjects/DeepST-KDD for_train/for_submit_data/feature_engine_data/try1.csv')
...
scaler.fit(read_csvfile[['temperature', 'humidity', 'wind_direction', 'wind_speed/kph','weather', 'NO2', 'CO', 'SO2', 'PM2.5', 'PM10', 'O3','hour','day','month','dayofweek']])  # Compute the minimum and maximum to be used for later scaling 这是个fit过程， 这个过程会 计算以后用于缩放的平均值和标准差， 记忆下来
data1 = pd.DataFrame(scaler.fit_transform(read_csvfile[['temperature', 'humidity', 'wind_direction', 'wind_speed/kph','weather', 'NO2', 'CO', 'SO2', 'PM2.5', 'PM10', 'O3','hour','day','month','dayofweek']]),columns=['temperature', 'humidity', 'wind_direction', 'wind_speed/kph','weather', 'NO2', 'CO', 'SO2', 'PM2.5', 'PM10', 'O3','hour','day','month','dayofweek'])  # Fit to data, then transform it. 使用记忆的数据进行转换
...
for i in range(len(read_csvfile)):
    #list_small=[]
    list_35.append([read_csvfile['temperature'][i],read_csvfile['humidity'][i],read_csvfile['wind_direction'][i],read_csvfile['wind_speed/kph'][i],read_csvfile['weather'][i],read_csvfile['NO2'][i],read_csvfile['CO'][i],read_csvfile['SO2'][i],read_csvfile['hour'][i],read_csvfile['day'][i],read_csvfile['month'][i],read_csvfile['dayofweek'][i]])
    list_35_air.append([read_csvfile['PM2.5'][i],read_csvfile['PM10'][i],read_csvfile['O3'][i]])
    list_pred_48_air.append([read_csvfile['temperature'][i],read_csvfile['humidity'][i],read_csvfile['wind_direction'][i],read_csvfile['wind_speed/kph'][i],read_csvfile['weather'][i]])
